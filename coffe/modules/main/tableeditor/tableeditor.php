 ...
 * @package coffe_cms
 */
class TableEditor_Module extends Coffe_Module
{
	 ...
	 * @var string
	 */
	protected $module_id = '_tableeditor';
	...
	public function processOneRecord()
	{
		...
		$this->table_ext = Coffe_ModuleManager::getTableEditor($this->table);
	...
	public function doEditOneRecord()
	{
		...
		$this->form = new Coffe_TableEditor($this->table);

		$this->form->setMethod('post')->setAction('');

		//hook
		Coffe_Event::call('TableEditor.beforeBuild', array(
				$this->table,
				&$this->table_ext,
				$this->primary,
				&$this->primary_value,
		...
		//hook
		Coffe_Event::call('TableEditor.afterBuild', array(
				$this->table,
				&$this->table_ext,
				$this->primary,
				&$this->primary_value,
	...
	public function addOneRecord()
	{
		if ($this->isPost()){
			...
			if ($this->form->isValid($post_data)){
				...
				$cancel = false;
				Coffe_Event::call('TableEditor.beforeAdd', array($this->table, $this->primary, &$data, &$cancel, &$this->flash));
				//отменили
				...
				//hook
				Coffe_Event::call('TableEditor.afterAdd', array($this->table, $this->primary, $this->primary_value, &$data, $cancel, &$this->flash));
	...
	public function editOneRecord()
	{
		if ($this->isPost()){
			...
			if ($this->form->isValid($post_data)){
				...
				$cancel = false;
				Coffe_Event::call('TableEditor.beforeUpdate', array($this->table, $this->primary, $this->primary_value,  &$data, &$cancel, &$this->flash));
				//отменили
				...
				Coffe_Event::call('TableEditor.afterUpdate', array($this->table, $this->primary, $this->primary_value,  &$data, $cancel, &$this->flash));
				if ($this->_POST('apply')){
					return $this->gotoEdit();
				}
				return $this->gotoBack();
	 ...
	 * Рисует одну форму
	 *
	 * @param Coffe_TableEditor $form
	 * @param string $table
	 */
	public function renderForm($form, $table)
	{
		$content = '<div class="tableeditor-'. $table .'">';
		$content .= $form->render();
		$content .= '</div';
		return $content;
	}
	...
	{
		$cancel = false;
		Coffe_Event::call('TableEditor.beforeOperation',array($this->table, $this->primary, $this->primary_value, $this->operation, &$cancel, $this->flash));
		$done = false;
		if (!$cancel){
			...
			if ($done){
				Coffe_Event::call('TableEditor.afterOperation',array($this->table, $this->primary, $this->primary_value, $this->operation, $cancel, $this->flash));
			}
		}
